<x-guest-layout>
    <div class="mb-8">
        ...
        <p class="mt-2 text-gray-500">Thanks for signing up! Before getting started, please verify your email address by clicking the link we just emailed to you.</p>
    </div>
    ...
    @if (session('status') == 'verification-link-sent')
        <div class="mb-6 flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-800 px-5 py-4 rounded-2xl">
            ...
            <span class="text-sm font-semibold">A new verification link has been sent to your email address.</span>
        </div>
    @endif
    ...
    <div class="flex items-center justify-between">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors">
                Log Out
            </button>
        </form>
    </div>
